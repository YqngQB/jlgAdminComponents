...
const tableGather = inject('$tableGather') as any
const setSortIndex = inject('$setSortIndex') as (id: string) => number
const slots = useSlots()
const attrs = useAttrs() as unknown as Record<string, any>
...
const props = defineProps({
	...
	formatter: [
		...
		Array,
		String
	] as PropType<VxeColumnPropTypes.Formatter>,
	children: {
...
const minColWidth = computed<string | number | undefined>(() => {
	...
	return (
		(attrs.width as string | number) ||
		(Math.ceil(Math.max(maxOne, state.getComputedWidth)) as number)
	)
})
		...
		nextTick(() => {
			...
			if (slots.default || attrs['cell-render'] || !!props.formatter) {
				...
				setTimeout(() => {
					...
					const nodes:HTMLElement[] = bodyWrapper.querySelectorAll(
						'.'.concat(
...
// 转换汉字为class支持的字母
function transChar(char:string) {
